...
class GENROE_AC7B_PSS2B
  ...
  OpalRT.Electrical.Control.Excitation.AC7B ac7b1(TR = TR_ex, KPR = KPR_ex, KIR = KIR_ex, KDR = KDR_ex, TDR = TDR_ex, VRMAX = VRMAX_ex, VRMIN = VRMIN_ex, KPA = KPA_ex, KIA = KIA_ex, VAMAX = VAMAX_ex, VAMIN = VAMIN_ex, KP = KP_ex, KL = KL_ex, KF1 = KF1_ex, KF2 = KF2_ex, KF3 = KF3_ex, TF3 = TF3_ex, KC = KC_ex, KD = KD_ex, KE = KE_ex, TE = TE_ex, VFEMAX = VFEMAX_ex, VEMIN = VEMIN_ex, E1 = E1_ex, S_E1 = S_E1_ex, E2 = E2_ex, S_E2 = S_E2_ex) annotation(Placement(visible = true, transformation(origin = {-54, 10}, extent = {{-12.5, -12.5}, {12.5, 12.5}}, rotation = 0)));
  Modelica.Blocks.Sources.Constant const(k = 0) annotation(Placement(visible = true, transformation(origin = {-92, 26}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
  input OpalRT.NonElectrical.Connector.InputInterfacePin dVREF annotation(Placement(visible = true, transformation(origin = {-112, -12}, extent = {{-5, -5}, {5, 5}}, rotation = 0), iconTransformation(origin = {-200, -80}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  OpalRT.Electrical.Control.Stabilizer.PSS2B pss2b1(M2 = M2_pss, TW1 = TW1_pss, TW2 = TW2_pss, T6 = T6_pss, TW3 = TW3_pss, TW4 = TW4_pss, T7 = T7_pss, KS2 = KS2_pss, KS3 = KS3_pss, T8 = T8_pss, T9 = T9_pss, KS1 = KS1_pss, T1 = T1_pss, T2 = T2_pss, T3 = T3_pss, T4 = T4_pss, VSTMAX = VSTMAX_pss, VSTMIN = VSTMIN_pss, M0 = M0_pss, M1 = M1_pss, M3 = M3_pss, M4 = M4_pss, M5 = M5_pss, initType = Modelica.Blocks.Types.Init.InitialOutput, ID = M_ID, T10 = T10_pss, T11 = T11_pss, VS1MAX = VS1MAX_pss, VS1MIN = VS1MIN_pss, VS2MAX = VS2MAX_pss, VS2MIN = VS2MIN_pss) annotation(Placement(visible = true, transformation(origin = {-100, 6}, extent = {{-15, -10}, {15, 10}}, rotation = 0)));
  input OpalRT.NonElectrical.Connector.InputInterfacePin TRIP annotation(Placement(visible = true, transformation(origin = {-20, 40}, extent = {{-5, -5}, {5, 5}}, rotation = 0), iconTransformation(origin = {-200, 80}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
equation
  connect(TRIP, genroe1.TRIP) annotation(Line(points = {{-20, 40}, {-19.7832, 40}, {-19.7832, 10}, {-20, 10}}));
  connect(genroe1.AccPower, pss2b1.PSS_AUX2[2]) annotation(Line(points = {{-10, -2.6}, {-2.17438, -2.6}, {-2.17438, -27.8321}, {-120.243, -27.8321}, {-120.243, -0.217438}, {-115.242, -0.217438}, {-115.242, -0.217438}}, color = {0, 0, 127}));
  connect(genroe1.SLIP, pss2b1.PSS_AUX2[1]) annotation(Line(points = {{-10, -8}, {-3.69645, -8}, {-3.69645, -25.2228}, {-120.243, -25.2228}, {-120.243, -0.217438}, {-115.46, -0.217438}, {-115.46, -0.217438}}, color = {0, 0, 127}));
  connect(pss2b1.VI2, pss2b1.VI) annotation(Line(points = {{-115, 2}, {-121.338, 2}, {-121.338, 7.80032}, {-115.271, 7.80032}, {-115.271, 7.80032}}, color = {0, 0, 127}));
  connect(genroe1.VI, pss2b1.VI2) annotation(Line(points = {{-10, 0}, {-3.64015, 0}, {-3.64015, -25.1344}, {-121.512, -25.1344}, {-121.512, 1.90674}, {-114.751, 1.90674}, {-114.751, 1.90674}}, color = {0, 0, 127}));
  connect(ac7b1.VI, genroe1.VI) annotation(Line(points = {{-41.5, 17.5}, {-3.89649, 17.5}, {-3.89649, -0.243531}, {-9.74123, -0.243531}, {-9.74123, -0.243531}}, color = {0, 0, 127}));
  connect(dVREF, ac7b1.dVREF) annotation(Line(points = {{-112, -12}, {-72.0851, -12}, {-72.0851, 2.43531}, {-66.4839, 2.43531}, {-66.4839, 2.43531}}));
  connect(genroe1.p, bus0) annotation(Line(points = {{-20, -10}, {-19.4417, -10}, {-19.4417, -28.4553}, {54, -28.4553}, {54, -28}}));
  connect(ac7b1.EFD0, genroe1.EFD0) annotation(Line(points = {{-41.5, 0}, {-30.2, 0}}, color = {0, 0, 127}));
  connect(ac7b1.EFD, genroe1.EFD) annotation(Line(points = {{-41.5, 2.5}, {-35.75, 2.5}, {-35.75, 2.8}, {-30, 2.8}}, color = {0, 0, 127}));
  connect(ac7b1.ETERM0, genroe1.ETERM0) annotation(Line(points = {{-41.5, 5}, {-35.75, 5}, {-35.75, 5}, {-30.2, 5}}, color = {0, 0, 127}));
  connect(ac7b1.EX_AUX, genroe1.EX_AUX) annotation(Line(points = {{-41.5, 8.25}, {-35.75, 8.25}, {-35.75, 8}, {-30.2, 8}}, color = {0, 0, 127}));
  connect(genroe1.XADIFD, ac7b1.XADIFD) annotation(Line(points = {{-10, 5}, {-6, 5}, {-6, 28}, {-72, 28}, {-72, 16.25}, {-66.5, 16.25}}, color = {0, 0, 127}));
  connect(pss2b1.VOTHSG, ac7b1.VOTHSG) annotation(Line(points = {{-85, 0}, {-76, 0}, {-76, 5.5}, {-66.5, 5.5}}, color = {0, 0, 127}));
  connect(ac7b1.VUEL, const.y) annotation(Line(points = {{-66.5, 12.5}, {-76, 12.5}, {-76, 26}, {-86.5, 26}}, color = {0, 0, 127}));
  connect(ac7b1.VOEL, const.y) annotation(Line(points = {{-66.5, 9}, {-76, 9}, {-76, 26}, {-86.5, 26}}, color = {0, 0, 127}));
  connect(pss2b1.PSS_AUX2, pss2b1.PSS_AUX) annotation(Line(points = {{-114.7, 0}, {-120, 0}, {-120, 6}, {-115, 6}}, color = {0, 0, 127}));
  connect(genroe1.PMECH0, genroe1.PMECH) annotation(Line(points = {{-30, -8}, {-32, -8}, {-32, -6}, {-30, -6}}, color = {0, 0, 127}));
  annotation(Diagram(coordinateSystem(extent = {{-200, -100}, {100, 100}}, preserveAspectRatio = true, initialScale = 0.1, grid = {2, 2})), experiment(StartTime = 0, StopTime = 20, Tolerance = 1e-06, Interval = 0.01), Icon(coordinateSystem(extent = {{-200, -100}, {100, 100}}, preserveAspectRatio = true, initialScale = 0.1, grid = {2, 2}), graphics={  Rectangle(origin = {-50, 0}, extent = {{-149.774, 100}, {149.774, -100}}), Text(origin = {-77.0165, 1.65977}, extent = {{-72.35, 23.36}, {106.974, -19.9431}}, textString = "GENROE_AC7B_PSS2B"), Text(origin = {77.128, -86.6058}, extent = {{-39.434, 24.942}, {7.19, -8.82}}, textString = "PIN"), Text(origin = {-147.75, 73.49}, extent = {{-39.43, 24.94}, {7.19, -8.82}}, textString = "TRIP"), Text(origin = {-145.52, -85.0977}, extent = {{-39.43, 24.94}, {20.1741, -8.82}}, textString = "dVREF")}));
end GENROE_AC7B_PSS2B;
