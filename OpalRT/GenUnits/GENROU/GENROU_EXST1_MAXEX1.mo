...
class GENROU_EXST1_MAXEX1
  ...
  OpalRT.Electrical.Control.OverExcitationLimiter.MAXEX1 maxex11(EFDRATED = EFDRATED_OEL, EFD1 = EFD1_OEL, TIME1 = TIME1_OEL, EFD2 = EFD2_OEL, TIME2 = TIME2_OEL, EFD3 = EFD3_OEL, TIME3 = TIME3_OEL, EFDDES = EFDDES_OEL, KMX = KMX_OEL, VLOW = VLOW_OEL) annotation(Placement(visible = true, transformation(origin = {-138, 60}, extent = {{-12.5, -12.5}, {12.5, 12.5}}, rotation = 0)));
  ...
  input OpalRT.NonElectrical.Connector.InputInterfacePin TRIP annotation(Placement(visible = true, transformation(origin = {-60, 68}, extent = {{-6, -6}, {6, 6}}, rotation = 0), iconTransformation(origin = {-198, 80}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  input OpalRT.NonElectrical.Connector.InputInterfacePin dVREF annotation(Placement(visible = true, transformation(origin = {-140, 0}, extent = {{-7, -7}, {7, 7}}, rotation = 0), iconTransformation(origin = {-200, -60}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
equation
  connect(exst11.VI, genrou1.VI) annotation(Line(points = {{-90, 46}, {-37.6401, 46}, {-37.6401, 31.9123}, {-48.0048, 31.9123}, {-48.0048, 31.9123}}, color = {0, 0, 127}));
  connect(dVREF, exst11.dVREF) annotation(Line(points = {{-140, 0}, {-115.375, 0}, {-115.375, 33.8216}, {-110.738, 33.8216}, {-110.738, 33.8216}}));
  connect(TRIP, genrou1.TRIP) annotation(Line(points = {{-60, 68}, {-59.4533, 68}, {-59.4533, 42}, {-58, 42}}));
  ...
  connect(exst11.VOEL, maxex11.VOEL) annotation(Line(points = {{-110, 39.2}, {-120, 39.2}, {-120, 60}, {-125, 60}}, color = {0, 0, 127}));
  connect(maxex11.EFD, genrou1.EFD) annotation(Line(points = {{-150.25, 67.5}, {-156, 67.5}, {-156, 76}, {-78, 76}, {-78, 34.8}, {-68, 34.8}}, color = {0, 0, 127}));
  connect(maxex11.XADIFD, genrou1.XADIFD) annotation(Line(points = {{-150.25, 52.5}, {-156, 52.5}, {-156, 46}, {-114, 46}, {-114, 56}, {-44, 56}, {-44, 37}, {-48, 37}}, color = {0, 0, 127}));
  annotation(Diagram(coordinateSystem(extent = {{-200, -100}, {100, 100}}, preserveAspectRatio = true, initialScale = 0.1, grid = {2, 2})), experiment(StartTime = 0, StopTime = 20, Tolerance = 1e-06, Interval = 0.01), Icon(coordinateSystem(extent = {{-200, -100}, {100, 100}}, preserveAspectRatio = true, initialScale = 0.1, grid = {2, 2}), graphics={  Rectangle(origin = {-50, 0}, extent = {{-149.774, 100}, {149.774, -100}}), Text(origin = {-59.8606, -2.02443}, extent = {{-119.955, 38.3301}, {140.913, -35.0366}}, textString = "GENROU_EXST1_MAXEX1"), Text(origin = {77.128, -86.6058}, extent = {{-39.434, 24.942}, {7.19, -8.82}}, textString = "PIN")}));
