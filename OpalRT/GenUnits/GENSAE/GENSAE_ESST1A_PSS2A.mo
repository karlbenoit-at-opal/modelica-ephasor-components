...
class GENSAE_ESST1A_PSS2A
  ...
  OpalRT.NonElectrical.Connector.PwPin bus0 annotation(Placement(visible = true, transformation(origin = {104, -38}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {104, -82}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  input OpalRT.NonElectrical.Connector.InputInterfacePin TRIP annotation(Placement(visible = true, transformation(origin = {34, 30}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {34, 30}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  OpalRT.Electrical.Machine.SynchronousMachine.GENSAE gensae1(IBUS = IBUS, ID = M_ID, P_gen = P_gen, Q_gen = Q_gen, Vt_abs = Vt_abs, Vt_ang = Vt_ang, SB = SB, fn = fn, ZSOURCE_RE = ZSOURCE_RE, Tdo_p = Tdo_p, Tdo_s = Tdo_s, Tqo_s = Tqo_s, H = H, D = D, Xd = Xd, Xq = Xq, Xd_p = Xd_p, Xd_s = Xd_s, Xl = Xl, S1 = S1, S12 = S12) annotation(Placement(visible = true, transformation(origin = {38, -4}, extent = {{-18, -18}, {18, 18}}, rotation = 0)));
  input OpalRT.NonElectrical.Connector.InputInterfacePin dVREF annotation(Placement(visible = true, transformation(origin = {-69, -15}, extent = {{-5, -5}, {5, 5}}, rotation = 0), iconTransformation(origin = {-80, 80}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  OpalRT.Electrical.Control.Excitation.ESST1A esst1a1(UEL = UEL_ex, VOS = VOS_ex, TR = TR_ex, VIMAX = VIMAX_ex, VIMIN = VIMIN_ex, TC = TC_ex, TB = TB_ex, TC1 = TC1_ex, TB1 = TB1_ex, KA = KA_ex, TA = TA_ex, VAMAX = VAMAX_ex, VAMIN = VAMIN_ex, VRMAX = VRMAX_ex, VRMIN = VRMIN_ex, KC = KC_ex, KF = KF_ex, TF = TF_ex, KLR = KLR_ex, ILR = ILR_ex) annotation(Placement(visible = true, transformation(origin = {-9, 11}, extent = {{-19, -19}, {19, 19}}, rotation = 0)));
  OpalRT.Electrical.Control.Stabilizer.PSS2A pss2a1(M2 = M2_pss, TW1 = TW1_pss, TW2 = TW2_pss, T6 = T6_pss, TW3 = TW3_pss, TW4 = TW4_pss, T7 = T7_pss, KS2 = KS2_pss, KS3 = KS3_pss, T8 = T8_pss, T9 = T9_pss, KS1 = KS1_pss, T1 = T1_pss, T2 = T2_pss, T3 = T3_pss, T4 = T4_pss, VSTMAX = VSTMAX_pss, VSTMIN = VSTMIN_pss, M0 = M0_pss, M1 = M1_pss, M3 = M3_pss, M4 = M4_pss, M5 = M5_pss, initType = Modelica.Blocks.Types.Init.InitialOutput, ID = M_ID) annotation(Placement(visible = true, transformation(origin = {-70, 4}, extent = {{-15, -10}, {15, 10}}, rotation = 0)));
  parameter Real noVOEL(fixed = false, start = 1);
  parameter Real noVUEL(fixed = false, start = 1);
  Modelica.Blocks.Sources.Constant constant1(k = noVUEL) annotation(Placement(visible = true, transformation(origin = {-52, 36}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
  Modelica.Blocks.Sources.Constant const(k = noVOEL) annotation(Placement(visible = true, transformation(origin = {-64, 22}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
initial equation
  noVOEL = Modelica.Constants.inf;
  noVUEL = if UEL_ex <> 1 then -Modelica.Constants.inf else 0;
equation
  connect(gensae1.VI, pss2a1.VI2) annotation(Line(points = {{56, -4}, {69.5524, -4}, {69.5524, -53.4599}, {-96.8279, -53.4599}, {-96.8279, 0}, {-85.6449, 0}, {-85.6449, 0}}, color = {0, 0, 127}));
  connect(gensae1.AccPower, pss2a1.PSS_AUX2[2]) annotation(Line(points = {{56, -8.68}, {63.5518, -8.68}, {63.5518, -48.8231}, {-94.1003, -48.8231}, {-94.1003, -2.18204}, {-84.8267, -2.18204}, {-84.8267, -2.18204}}, color = {0, 0, 127}));
  connect(gensae1.SLIP, pss2a1.PSS_AUX2[1]) annotation(Line(points = {{56, -18.4}, {63.5518, -18.4}, {63.5518, -48.8231}, {-94.1003, -48.8231}, {-94.1003, -2.18204}, {-85.6449, -2.18204}, {-85.6449, -2.18204}}, color = {0, 0, 127}));
  connect(pss2a1.VI2, pss2a1.VI) annotation(Line(points = {{-85, -2.22045e-16}, {-91.1, -2.22045e-16}, {-91.1, 6.27335}, {-85.3722, 6.27335}, {-85.3722, 6.27335}}, color = {0, 0, 127}));
  connect(dVREF, esst1a1.dVREF) annotation(Line(points = {{-69, -15}, {-36.2764, -15}, {-36.2764, -1.09102}, {-27.821, -1.09102}, {-27.821, -1.09102}}));
  connect(esst1a1.VI, gensae1.VI) annotation(Line(points = {{10, 22.4}, {69.5279, 22.4}, {69.5279, -3.86266}, {56.6524, -3.86266}, {56.6524, -3.86266}}, color = {0, 0, 127}));
  connect(TRIP, gensae1.TRIP) annotation(Line(points = {{34, 30}, {38.5277, 30}, {38.5277, 14}, {38, 14}}));
  connect(gensae1.p, bus0) annotation(Line(points = {{38, -22}, {38.6279, -22}, {38.6279, -35.5589}, {104, -35.5589}, {104, -38}}));
  connect(esst1a1.EFD0, gensae1.EFD0) annotation(Line(points = {{10, -4.2}, {14, -4.2}, {14, -4}, {19.64, -4}}, color = {0, 0, 127}));
  connect(esst1a1.EFD, gensae1.EFD) annotation(Line(points = {{10, -0.4}, {14, -0.4}, {14, 1.04}, {20, 1.04}}, color = {0, 0, 127}));
  connect(esst1a1.ETERM0, gensae1.ETERM0) annotation(Line(points = {{10, 3.4}, {14, 3.4}, {14, 5}, {19.64, 5}}, color = {0, 0, 127}));
  connect(esst1a1.EX_AUX, gensae1.EX_AUX) annotation(Line(points = {{10, 8.34}, {14, 8.34}, {14, 10.4}, {19.64, 10.4}}, color = {0, 0, 127}));
  connect(esst1a1.VOTHSG, pss2a1.VOTHSG) annotation(Line(points = {{-28, 4.16}, {-42, 4.16}, {-42, -2}, {-55, -2}}, color = {0, 0, 127}));
  connect(esst1a1.XADIFD, gensae1.XADIFD) annotation(Line(points = {{-28, 20.5}, {-36, 20.5}, {-36, 44}, {66, 44}, {66, 5}, {56, 5}}, color = {0, 0, 127}));
  connect(pss2a1.PSS_AUX2, pss2a1.PSS_AUX) annotation(Line(points = {{-84.7, -2}, {-94, -2}, {-94, 4}, {-85, 4}}, color = {0, 0, 127}));
  connect(constant1.y, esst1a1.VUEL) annotation(Line(points = {{-46.5, 36}, {-40, 36}, {-40, 14.8}, {-28, 14.8}}, color = {0, 0, 127}));
  connect(esst1a1.VOEL, const.y) annotation(Line(points = {{-28, 9.48}, {-48, 9.48}, {-48, 22}, {-58.5, 22}}, color = {0, 0, 127}));
  connect(gensae1.PMECH0, gensae1.PMECH) annotation(Line(points = {{20, -18.4}, {18, -18.4}, {18, -18}, {14, -18}, {14, -14.8}, {20, -14.8}}, color = {0, 0, 127}));
  annotation(Diagram(coordinateSystem(extent = {{-100, -100}, {100, 100}}, preserveAspectRatio = true, initialScale = 0.1, grid = {2, 2})), Icon(coordinateSystem(extent = {{-100, -100}, {100, 100}}, preserveAspectRatio = true, initialScale = 0.1, grid = {2, 2}), graphics={  Rectangle(extent = {{-100, 100}, {100, -100}}), Text(origin = {-4, 2}, extent = {{-58, 29}, {58, -29}}, textString = "GENSAE_ESST1A_PSS2A"), Text(origin = {70, -80}, extent = {{-30, 14}, {30, -14}}, textString = "PIN")}));
end GENSAE_ESST1A_PSS2A;
