...
block PI_NonWindupLimit2 "2-nd implementation of PI controller with non-windup limiter"
  ...
  OpalRT.NonElectrical.Math.Continuous.TransferFunctionNonWindup.Integrator_NonWindupLimit non_windup_integrator1(KI = KI, VRMAX = MAX, VRMIN = MIN, y_init = if KI <> 0 then y_start else 0) annotation(Placement(visible = true, transformation(origin = {-40, 20}, extent = {{-15, -15}, {15, 15}}, rotation = 0)));
equation
