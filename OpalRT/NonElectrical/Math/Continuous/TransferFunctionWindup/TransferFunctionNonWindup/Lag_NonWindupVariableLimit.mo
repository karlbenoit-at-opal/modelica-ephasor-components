...
block Lag_NonWindupVariableLimit "Lag block with anti wind-up variable limit"
  ...
  TransferFunctionNonWindup.Integrator_NonWindupVariableLimit non_windup_integrator_var1(y_init = y_init, KI = if TI <> 0 then 1 / TI else 1) annotation(Placement(visible = true, transformation(origin = {40, 0}, extent = {{-15, -15}, {15, 15}}, rotation = 0)));
