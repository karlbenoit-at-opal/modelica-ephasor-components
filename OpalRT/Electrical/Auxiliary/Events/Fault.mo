...
block Fault "Transitory short-circuit on a node. Shunt impedance connected only during a specified interval of time. 2014/03/10"
...
equation
  ...
  p.ir = if time <= t1 then 0 else if time <= t2 then -(R * p.vr + X * p.vi) / (R * R + X * X) else 0;
