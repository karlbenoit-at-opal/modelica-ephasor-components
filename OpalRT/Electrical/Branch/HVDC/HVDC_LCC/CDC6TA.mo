...
model CDC6TA
  ...
  parameter Real TVRDC = 1 "rectifier dc voltage transducer time constant (sec)";
  ...
  OpalRT.NonElectrical.Math.Continuous.TransferFunction.Lag lag2(T = TVRDC, y_start = vdcr0) annotation(Placement(visible = true, transformation(origin = {0, 0}, extent = {{5, -5}, {-5, 5}}, rotation = 0)));
...
equation
  connect(lag2.y, multiplexer2.u[2]) annotation(Line(points = {{-5, 0}, {-54.7945, 0}, {-54.7945, 19.3888}, {-54.7945, 19.3888}}, color = {0, 0, 127}));
  connect(bridge_line_simulation_logic1.VdcR, lag2.u) annotation(Line(points = {{25, -80}, {7.58693, -80}, {7.58693, 0.210748}, {5.2687, 0.210748}, {5.2687, 0.210748}}, color = {0, 0, 127}));
...
This model is similar to CDC6TA in PSS/e, however not all the features are implemented in this version. The limitations are summerized below:
...
The block diagram of the model is shown below. This figure is different from the one in pss/e documentation. The modificatons are made to match the exact behavoir.

<img src=\"modelica://OpalRT/resource/Electrical/BRANCH/HVDC/HVDC_LCC/CDC6TA.png\" width=\"80%\">


</html>"), Icon(coordinateSystem(extent = {{-100, -100}, {100, 100}}, preserveAspectRatio = true, initialScale = 0.1, grid = {2, 2}), graphics={  Rectangle(origin = {0.405954, -0.541272}, extent = {{-100, 99.594}, {100, -99.594}}), Text(origin = {-8.98816, -27.1637}, extent = {{-72.94, 22.46}, {72.94, -22.46}}, textString = "CDC6TA"), Text(origin = {-80.11, 60.89}, extent = {{-13.8, 6.22}, {13.8, -6.22}}, textString = "d_SETVL"), Text(origin = {-75.7795, 3.38636}, extent = {{-18.94, 8.25}, {14.0686, -3.37855}}, textString = "d_VSCHED"), Text(origin = {-75.5114, -92.5594}, extent = {{-17.86, 5.95}, {17.86, -5.95}}, textString = "inverterp"), Text(origin = {75.3714, -91.4794}, extent = {{-22.06, 6.5}, {22.06, -6.5}}, textString = "rectifp")}));
end CDC6TA;
